...
Tests assert; validators report. The suite in `validation/` produces
...
3. **Frozen MATLAB behavioural fixtures.** Traces exported once from the MATLAB
   implementation and committed under `tests/fixtures/`, replayed to
   check that the Python model lands in the same *distributional* neighbourhood.

### 9.2 The frozen fixtures

`tests/fixtures/` holds MATLAB `.mat` files exported once and committed:
...
`tests/fixtures/oracle/` and skips its oracle arm when they are absent, so
