...
Fixture format
--------------
``tests/fixtures/oracle/coin_trace_seed<seed>.mat``, readable with
``scipy.io.loadmat(..., squeeze_me=True)``. See
``tests/fixtures/oracle/README.md`` for the full variable list and the
...
def load_trace(seed=DEFAULT_SEED, fixture_dir=None):
    ...
    if not path.is_file():
        raise FileNotFoundError(
            ...
            "tests/fixtures/oracle/README.md)" % path
        )
