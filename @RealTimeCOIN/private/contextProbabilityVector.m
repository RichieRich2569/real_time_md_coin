...
%              exists). Non-finite entries are treated as zero.
    ...
    % Integer-centred bin edges 0.5, 1.5, ..., (Cmax+1)+0.5 so that histcounts
    % places each context label k in its own bin k. Cmax+1 here is max_contexts+1
    % (novel slot), giving max_contexts+1 bins.
    edges = 0.5:1:(obj.max_contexts + 1.5);
    weights = contextProbabilityVectorCore(kind, ...
        @() obj.globalContextWeights(obj.D.predicted_probabilities, alignment), ...
        @() obj.globalContextWeights(obj.D.responsibilities, alignment), ...
        @() obj.globalSampledContexts(alignment), ...
        edges);
