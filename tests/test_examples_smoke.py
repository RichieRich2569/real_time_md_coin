"""End-to-end smoke test for the example scripts in ``examples``.
...
#: ``examples``, resolved from this file so the test is cwd-independent.
...
def test_example_script_runs_headless(script, tmp_path):
    ...
    # process. Put the repo root on the child's path so a plain source checkout (no
