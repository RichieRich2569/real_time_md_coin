...
> **Provenance note.** The MATLAB capture scripts referenced throughout this
> file (`COIN.m`, `validation/*.m`, `tests/+equiv/*.m`) are no longer on this
> branch — this branch carries only the Python implementation. They live on the
> repository's `main`/`experimental` branches and in this branch's
> pre-restructure history; regenerating any fixture requires a MATLAB checkout
> from there.
