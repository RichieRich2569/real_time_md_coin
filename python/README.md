...
`realtimecoin` is a Python translation of the MATLAB `@RealTimeCOIN` class folder
in this repository: a real-time (sequential) implementation of the COIN
...
(`pip install -e python/` without the `[test]` extra if you do not need
...
This package is a translation of the MATLAB `@RealTimeCOIN` implementation in
this repository, which in turn re-implements the COIN model of Heald, Lengyel &
